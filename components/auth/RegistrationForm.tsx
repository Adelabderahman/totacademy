...
export const RegistrationForm: React.FC<RegistrationFormProps> = ({
  ...
  const { login, register, loginWithGoogle, quickDemoLogin } = useUserAccount();
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.success) {
        setErrorMessage(res.error || 'فشلت عملية إنشاء الحساب.');
      ...
      const msg =
        language === 'ar'
          ? `مرحباً بك يا ${registerName}! تم إنشاء وتوثيق حسابك في الأكاديمية بنجاح عبر Firebase.`
      ...
      <div key={mode} className="animate-form-smooth">
        {mode === 'login' ? (
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-3 sm:space-y-3.5">
            ...
            >
              {isSubmitting
                ? (language === 'ar' ? 'جاري تسجيل الدخول عبر Firebase...' : 'Authenticating with Firebase...')
            ...
            <div className="relative flex py-1 items-center">
              ...
              <span className="flex-shrink mx-3 text-[10px] sm:text-xs text-slate-400 font-medium">
                {language === 'ar' ? 'أو عبر الخيارات السريعة' : 'Or quick options'}
              </span>
              <div className="flex-grow border-t border-slate-200"></div>
            </div>

            {/* أزرار الدخول السريع و Google */}
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                onClick={async () => {
                  ...
                  if (res.success && onSuccess) {
                    onSuccess(language === 'ar' ? 'تم تسجيل الدخول بحساب Google!' : 'Signed in with Google!');
                  } else if (res.error) {
                    setErrorMessage(res.error);
                  }
                }}
                disabled={isSubmitting}
                className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold transition-all cursor-pointer shadow-2xs active:scale-95"
              >
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const msg = language === 'ar' ? 'تم الدخول السريع بحساب المدرب المعتمد التجريبي!' : 'Quick demo trainer access activated!';
                  setStatusMessage(msg);
                  if (onSuccess) onSuccess(msg);
                }}
                className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl border border-primary-blue/30 bg-primary-blue/5 hover:bg-primary-blue/10 text-primary-blue text-xs font-semibold transition-all cursor-pointer shadow-2xs active:scale-95"
              >
                <span>⚡</span>
                <span>{language === 'ar' ? 'دخول تجريبي فوري' : 'Quick Demo'}</span>
              </button>
            </div>
          ...
          <form onSubmit={handleRegisterSubmit} className="space-y-3.5">
            ...
            >
              {isSubmitting
                ? (language === 'ar' ? 'جاري إنشاء الحساب وتوثيقه...' : 'Creating account...')
            ...
            <div className="relative flex py-0.5 items-center">
              <div className="flex-grow border-t border-slate-200"></div>
              <span className="flex-shrink mx-3 text-[10px] sm:text-xs text-slate-400 font-medium">
                {language === 'ar' ? 'أو التسجيل السريع' : 'Or quick registration'}
              </span>
              <div className="flex-grow border-t border-slate-200"></div>
            </div>

            {/* أزرار التسجيل السريع و Google */}
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={async () => {
                  ...
                  setIsSubmitting(false);
                  if (res.success && onSuccess) {
                    onSuccess(language === 'ar' ? 'تم التسجيل وتوثيق الحساب بـ Google بنجاح!' : 'Signed up with Google successfully!');
                  } else if (res.error) {
                    setErrorMessage(res.error);
                  }
                }}
              ...
              <button
                type="button"
                onClick={() => {
                  quickDemoLogin(registerRole.includes('trainer') ? 'trainer' : 'trainee');
                  const msg = language === 'ar' ? 'تم إنشاء وتفعيل الحساب الفوري بنجاح!' : 'Quick account activated!';
                  setStatusMessage(msg);
                  if (onSuccess) onSuccess(msg);
                }}
                className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl border border-primary-green/30 bg-primary-green/5 hover:bg-primary-green/10 text-primary-green text-xs font-semibold transition-all cursor-pointer shadow-2xs active:scale-95"
              >
                <span>⚡</span>
                <span>{language === 'ar' ? 'تسجيل فوري مباشر' : 'Quick Instant Sign-up'}</span>
              </button>
            </div>
