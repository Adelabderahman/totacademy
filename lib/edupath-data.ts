...
export const finalExamQuestions: Record<string, LocalizedString[]> = {
  m1: [
    { ar: "كيف توظف مبدأ 'الحاجة إلى المعرفة' عند تصميم دورة لجمهور ذي خبرة عملية واسعة؟ *", en: "How do you apply the 'need to know' principle when designing a course for experienced audiences? *", fr: "Comment appliquez-vous le principe du 'besoin de savoir' pour un public expérimenté ? *" },
    { ar: "اقترح استراتيجية تطبيقية للتعامل مع انخفاض الانتباه بعد أول 20 دقيقة من الجلسة التدريبية. *", en: "Suggest a practical strategy to handle attention drops after the first 20 minutes of a session. *", fr: "Suggérez une stratégie pour gérer la baisse d'attention après les 20 premières minutes. *" },
    { ar: "كيف تدمج تقنيات 'التكرار المتباعد' و'الاسترجاع النشط' في تدريب ممتد لضمان ترسيخ المهارات؟ *", en: "How do you integrate 'spaced repetition' and 'active recall' in extended training to anchor skills? *", fr: "Comment intégrez-vous la 'répétition espacée' et le 'rappel actif' pour consolider les acquis ? *" },
    { ar: "كيف تعدل أسلوبك البيداغوجي إذا أظهر التشخيص القبلي أن أغلبية المتدربين ينتمون للنمط 'الحركي'؟ *", en: "How do you adapt your pedagogy if pre-assessment reveals that the majority are 'kinesthetic' learners? *", fr: "Comment adaptez-vous votre pédagogie pour une majorité d'apprenants 'kinesthésiques' ? *" },
    { ar: "حلل دور 'التغذية الراجعة التكوينية' كأداة للتقييم وتأثيرها المباشر على الدافعية الذاتية للمتدربين. *", en: "Analyze the role of 'formative feedback' as an assessment tool and its direct impact on trainee motivation. *", fr: "Analysez le rôle du 'feedback formatif' et son impact direct sur la motivation intrinsèque des apprenants. *" }
  ],
  m2: [
    { ar: "كيف توظف لغة الجسد وتوزيع النظرات بنظام W لبناء هيبة وألفة فورية في القاعة؟ *", en: "How do you use body language and W-pattern eye contact to establish immediate presence and rapport? *", fr: "Comment utilisez-vous le langage corporel et le balayage visuel en W pour créer présence et confiance ? *" },
    { ar: "حدد آليات التحكم في طبقات الصوت والوقفات الدرامية لكسر الرتابة وإبراز الرسائل الجوهرية. *", en: "Detail voice pitch control and dramatic pauses to break monotony and highlight key takeaways. *", fr: "Détaillez le contrôle vocal et les pauses dramatiques pour rompre la monotonie et marquer les messages clés. *" },
    { ar: "كيف تبني وتوظف قصة تدريبية متكاملة الأركان (بطل، صراع، حل) لترسيخ مفهوم إداري معقد؟ *", en: "How do you structure an applied training story (hero, conflict, resolution) to anchor a complex concept? *", fr: "Comment structurez-vous une histoire de formation (héros, conflit, résolution) pour ancrer un concept complexe ? *" },
    { ar: "اشرح منهجية التعامل الدبلوماسي الحازم مع المتدرب 'المتعالي' دون كسر هيبة القاعة أو إحراجه. *", en: "Explain the diplomatic strategy to handle an 'arrogant' participant without derailing the room. *", fr: "Expliquez la stratégie diplomatique pour recadrer un participant 'arrogant' avec fermeté et respect. *" },
    { ar: "ما خطوات تدريب النفس على التغلب على رهبة المسرح قبل الصعود للمنصة بـ 10 دقائق؟ *", en: "What are the psychological steps to manage stage fright 10 minutes prior to stepping on stage? *", fr: "Quelles sont les étapes psychologiques pour gérer le trac 10 minutes avant de monter sur scène ? *" }
  ],
  m3: [
    { ar: "قارن بدقة بين مراحل نموذج ADDIE الخمس ونماذج التصميم السريع الحديثة. *", en: "Rigorously compare the 5 stages of ADDIE with modern rapid instructional design models. *", fr: "Comparez rigoureusement les 5 phases d'ADDIE avec les modèles modernes de design rapide. *" },
    { ar: "صغ هدفاً تدريبياً مركباً يستوفي شروط SMART ويستهدف المستوى الإبداعي في تصنيف بلوم. *", en: "Formulate a SMART training objective targeting the 'Create' tier in Bloom's taxonomy. *", fr: "Formulez un objectif SMART ciblant le niveau 'Création' de la taxonomie de Bloom. *" },
    { ar: "كيف توظف استراتيجية 'التجزئة' (Chunking) لهندسة حقيبة تدريبية مكثفة من 3 أيام؟ *", en: "How do you apply chunking to architect an intensive 3-day training package? *", fr: "Comment appliquez-vous le chunking pour concevoir un kit de formation intensif de 3 jours ? *" },
    { ar: "ما العناصر الإلزامية التي يجب تضمينها في دليل المدرب (Trainer Manual) لضمان قابلية تشغيله؟ *", en: "What mandatory components must be included in a Trainer Manual to guarantee turn-key execution? *", fr: "Quels éléments obligatoires doivent figurer dans le Manuel du Formateur pour en garantir l'exécution ? *" },
    { ar: "صمم سلم تقييم (Rubric) من 4 مستويات لقياس كفاءة إعداد عرض تقديمي فعال. *", en: "Design a 4-tier evaluation rubric to assess competency in delivering an effective presentation. *", fr: "Concevez une grille d'évaluation à 4 niveaux pour mesurer la compétence de présentation. *" }
  ],
  m4: [
    { ar: "كيف تطبق ميكانيكا التلعيب (نقاط، أوسمة، مسارات) في برنامج مخصص للقيادات العليا؟ *", en: "How do you implement gamification mechanics (points, badges, leaderboards) in an executive program? *", fr: "Comment appliquez-vous la ludification (points, badges, classements) pour des cadres supérieurs ? *" },
    { ar: "حدد معايير صياغة مشكلة واقعية محفزة لتطبيق استراتيجية التعلم القائم على المشكلات (PBL). *", en: "Define the criteria for formulating an authentic problem for Problem-Based Learning (PBL). *", fr: "Définissez les critères d'un problème authentique pour l'apprentissage par résolution de problèmes (PBL). *" },
    { ar: "كيف تصمم سيناريو محاكاة واقعي متعدد المسارات (Branching Scenario) لتدريب خدمة العملاء؟ *", en: "How do you design a realistic branching simulation scenario for customer service training? *", fr: "Comment concevez-vous un scénario de simulation à embranchements pour le service client ? *" },
    { ar: "ما الضوابط التي تمنع تحول الترفيه التعليمي (Edutainment) إلى مضيعة للوقت بلا عائد معرفي؟ *", en: "What safeguards ensure edutainment does not devolve into frivolous time-wasting without learning? *", fr: "Quels garde-fous empêchent l'edutainment de dériver en divertissement stérile sans apprentissage ? *" },
    { ar: "وضح بالخطوات كيفية توظيف التفكير التصميمي (Design Thinking) لحل مشكلة تسرب المتدربين. *", en: "Step-by-step, explain using Design Thinking to solve the challenge of trainee drop-out. *", fr: "Détaillez par étapes l'usage du Design Thinking pour résoudre l'abandon des apprenants. *" }
  ],
  m5: [
    { ar: "كيف تدمج نماذج الذكاء الاصطناعي التوليدي في مرحلة إعداد السيناريوهات والأنشطة التدريبية؟ *", en: "How do you integrate generative AI models in creating training scenarios and activities? *", fr: "Comment intégrez-vous l'IA générative dans la création de scénarios et d'activités pédagogiques ? *" },
    { ar: "قارن بين منصات LMS التقليدية ومنصات تجربة التعلم الحديثة (LXP) من حيث تفاعل المتعلم. *", en: "Contrast traditional LMS platforms with modern LXPs regarding learner engagement. *", fr: "Comparez les LMS traditionnels et les LXP modernes en termes d'engagement des apprenants. *" },
    { ar: "ما هي البروتوكولات الأمنية والاحترازية لتأمين قاعة تدريب افتراضية وحماية بيانات المشاركين؟ *", en: "What security protocols are required to safeguard a virtual classroom and participant data? *", fr: "Quels protocoles de sécurité sont nécessaires pour protéger une classe virtuelle et les données ? *" },
    { ar: "كيف توظف أدوات الأتمتة (No-Code/Zapier) لبناء رحلة متابعة مستمرة بعد انتهاء التدريب؟ *", en: "How do you deploy no-code automation (Zapier) to build an ongoing post-training follow-up workflow? *", fr: "Comment déployez-vous l'automatisation sans code (Zapier) pour structurer le suivi post-formation ? *" },
    { ar: "حدد المعايير الأخلاقية الواجب التزامها عند إدراج محتوى مولد بالذكاء الاصطناعي في دوراتك. *", en: "Outline the ethical guidelines required when incorporating AI-generated content in your courses. *", fr: "Exposez les règles éthiques à respecter lors de l'intégration de contenu généré par l'IA. *" }
  ],
  m6: [
    { ar: "اشرح بالتفصيل معادلة العائد على الاستثمار (ROI) وفق منهجية جاك فيليبس موضحاً التكاليف الصافية. *", en: "Detail the Jack Phillips ROI equation, elaborating on fully-loaded costs vs net monetary benefits. *", fr: "Détaillez l'équation du ROI de Jack Phillips, en expliquant les coûts complets vs bénéfices nets. *" },
    { ar: "ما هي الآليات الإحصائية المعتمدة لعزل أثر التدريب عن المؤثرات الخارجية الأخرى في السوق؟ *", en: "What statistical mechanisms isolate training effects from external market fluctuations? *", fr: "Quels mécanismes statistiques permettent d'isoler l'impact de la formation des facteurs externes ? *" },
    { ar: "كيف تحول مخرجات المهارات الناعمة (مثل إدارة الضغوط) إلى قيم مالية قابلة للقياس الكمي؟ *", en: "How do you monetize soft skill outcomes (e.g. stress resilience) into quantifiable currency? *", fr: "Comment monétisez-vous les soft skills (ex: gestion du stress) en valeurs financières quantifiables ? *" },
    { ar: "صمم لوحة قيادة افتراضية (Dashboard) تتضمن 5 مؤشرات حيوية لقياس أثر برنامج تدريبي للقيادة. *", en: "Design a mock dashboard with 5 core KPIs to measure the impact of a leadership program. *", fr: "Concevez un tableau de bord avec 5 KPI vitaux pour mesurer l'impact d'un programme de leadership. *" },
    { ar: "كيف تقدم تقريراً تنفيذياً للإدارة العليا يقنعهم بزيادة ميزانية التدريب السنوية بناءً على الأرقام؟ *", en: "How do you pitch an executive impact report to senior leaders to justify budget expansion based on data? *", fr: "Comment présentez-vous un rapport d'impact exécutif pour justifier l'extension budgétaire par les chiffres ? *" }
  ],
  m7: [
    { ar: "ما هي مرتكزات الحوكمة الرشيدة للمؤسسات التدريبية لضمان الاستدامة التشغيلية والمالية؟ *", en: "What are the foundations of good governance for training institutions to ensure operational sustainability? *", fr: "Quels sont les piliers d'une bonne gouvernance garantissant la pérennité opérationnelle et financière ? *" },
    { ar: "حلل مراحل الحصول على اعتماد معايير الجودة الدولية (مثل ISO 10015) الخاصة بإدارة التدريب. *", en: "Analyze the milestones to obtain international ISO 10015 quality accreditation for training management. *", fr: "Analysez les étapes d'obtention de l'accréditation qualité internationale ISO 10015. *" },
    { ar: "كيف تبني مصفوفة الصلاحيات والمسؤوليات (RACI) لإدارة مشروع تدريبي استراتيجي ضخم؟ *", en: "How do you construct a RACI matrix to govern a large-scale strategic training deployment? *", fr: "Comment construisez-vous une matrice RACI pour piloter un projet de formation stratégique majeur ? *" },
    { ar: "ما استراتيجيات تسعير الحقائب والخدمات التدريبية الموجهة للشركات الكبرى (B2B Pricing)؟ *", en: "What are the core B2B value-based pricing strategies for corporate training contracts? *", fr: "Quelles sont les stratégies de tarification B2B basée sur la valeur pour les grands comptes ? *" },
    { ar: "كيف تبني شراكات استراتيجية مع الجامعات والمراكز الدولية تعزز الموثوقية وتفتح أسواقاً جديدة؟ *", en: "How do you build strategic partnerships with universities to boost credibility and open new markets? *", fr: "Comment bâtir des partenariats stratégiques universitaires pour asseoir la crédibilité et conquérir des marchés ? *" }
  ],
  m8: [
    { ar: "حدد بدقة مفهوم التخصص الدقيق (Niche) وكيف يصنع للمدرب مكانة ريادية لا يمكن منافستها. *", en: "Define deep niche positioning and how it establishes an unassailable leadership authority for a trainer. *", fr: "Définissez le positionnement de niche et comment il forge une autorité incontestable pour le formateur. *" },
    { ar: "ما هي استراتيجية صناعة المحتوى على منصة LinkedIn لتحويل المتابعين إلى عقود تدريبية واستشارية؟ *", en: "What is the LinkedIn content strategy to convert professional followers into high-ticket contracts? *", fr: "Quelle stratégie de contenu LinkedIn convertit les abonnés professionnels en contrats à haute valeur ? *" },
    { ar: "كيف توظف الدليل الاجتماعي (Social Proof) وقصص نجاح الخريجين لبناء سمعة مهنية ذات موثوقية؟ *", en: "How do you deploy social proof and alumni success stories to cement an authoritative reputation? *", fr: "Comment déployez-vous la preuve sociale et les réussites d'anciens pour asseoir votre réputation ? *" },
    { ar: "وضح آليات حماية الملكية الفكرية لحقائبك ومؤلفاتك التدريبية وصياغة عقود الترخيص (Licensing). *", en: "Detail IP protection mechanisms for training packages and structuring IP licensing contracts. *", fr: "Détaillez les mécanismes de protection de la propriété intellectuelle et la structuration de contrats de licence. *" },
    { ar: "ما هي خارطة الطريق لتحويل خبرتك كمدرب من الاعتماد على الوقت إلى بناء أصول رقمية مدرة للدخل؟ *", en: "What is the roadmap to transition from trading time for money to building scalable digital revenue assets? *", fr: "Quelle feuille de route permet de passer de la vente de son temps à la création d'actifs numériques rentables ? *" }
  ]
};
