...
import { foundationLessonsDB } from '@/lib/foundation-curriculum-data';
import { getFoundationQuizzes } from '@/lib/foundation-quizzes-data';
...
export const allLessonsDB: Record<string, LessonData[]> = {
  // Foundation Modules 1 to 8 (Fully populated comprehensive curriculum data)
  ...foundationLessonsDB,
...
export function getQuizzesForModule(moduleId: string, lang: LangKey): QuizItem[] {
  if (moduleId.startsWith('module_')) {
    const fq = getFoundationQuizzes(moduleId, lang);
    if (fq && fq.length > 0) return fq;
  }
